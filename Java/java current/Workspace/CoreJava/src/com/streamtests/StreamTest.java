...
public class StreamTest {
	...
	public static void main(String[] args) {
		...
		System.out.println("\n Min value is");
		Optional<Integer> minValue = transactions.stream().min(Comparator.comparing(Transaction::getValue)).map((Transaction t)-> t.getValue());
		System.out.println(minValue.get());
		//15.	Find out which user has posted maximum comments.
		System.out.println("\n ");
		Optional<Entry<String, Long>> maxUserCommenter = news.stream().collect(Collectors.groupingBy( News::getCommentByUser,Collectors.counting())).entrySet().stream().sorted(Entry.comparingByKey()).max(Entry.comparingByValue());					
		System.out.println(maxUserCommenter.get().getKey());
	}
