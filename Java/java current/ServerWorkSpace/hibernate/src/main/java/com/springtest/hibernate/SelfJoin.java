...
public class SelfJoin {
    ...
    public static void managerNotNull(SessionFactory sessionFactory){
    	...
    	Session session = sessionFactory.openSession();
    	Transaction tobj = session.beginTransaction();
    	Query query2 = session.createQuery("from EmployeeManager");
    	List<EmployeeManager> Manager = query2.list();
    	HashSet<Long> mid = new HashSet<>();
    	
    	for(EmployeeManager e : Manager){
    		if(e.getManager()!=null){
    			mid.add(e.getManager().getEid());
    		}    	
    	}
    	List<Long> mid2 = mid.stream().collect(Collectors.toList());
    	
    	List<EmployeeManager> Managers = query2.list();
    	System.out.println("********MANAGER LIST***********");
    	System.out.println("EID\tENAME\tMANAGER_ID");
    	for(EmployeeManager e : Managers){
    		if(mid2.contains(e.getEid())==true){
    	...
    	System.out.println("********EMPLOYEE WITH MANAGER LIST***********");
    	System.out.println("EID\tENAME\tMANAGER_ID");
    	for(EmployeeManager e : underManager){
    	System.out.println(e.getEid()+"\t"+e.getName()+"\t"+e.getManager().getEid());
